...
import OrderList from './StandbyOrdersList';
...
function StandbyOrders() {
    ...
    useEffect(() => {
        const fetchMyOrders = async(e) => {
            const response = await fetch('http://localhost:3000/api/standbyorders', {
            ...
            <tr className="border-b-4 border-black">
                ...
                {orders.map(orderItem => (
                    <tr key={orderItem.id}>
                        ...
                        <Link to="/standbyorder"><th className="pt-5 w-1/8">Pogledaj proizvode</th></Link>
    ...
    <Route exact strict path="/standbyorder">
        <OrderList/>
    </Route>
    </div>
    </Switch>
