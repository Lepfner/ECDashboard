...
import OrderList from './ProcessedOrdersList';
...
function ProcessedOrders() {
    ...
    useEffect(() => {
        const fetchMyOrders = async(e) => {
            const response = await fetch('http://localhost:3000/api/processedorders', {
        ...
        <table className="ml-6 w-skoro text-left">
            ...
            {orders.map(orderItem => (
                <tr key={orderItem.id}>
                    ...
                    <Link to="/processedorder"><th className="pt-5 w-1/8">Pogledaj proizvode</th></Link>
    ...
    <Route exact strict path="/processedorder">
        <OrderList/>
    </Route>
    </div>
    </Switch>
